...
def predict_multiplier(hash_str, historical_data):
    ...
    if predicted_color == "Red":
        ...
        # Green multiplier prediction based on historical data and safety margin
        first_char = hash_str[0]
        char_data = historical_data[historical_data["Hash"].str.startswith(first_char)]

        if not char_data.empty:
            predicted_multiplier = char_data["Multiplier"].mean() + 1.00  # Add a safety margin of 1.00

        else:
            predicted_multiplier = 1.5  # Default value if no data is available

        # Consider 2-character and 3-character sequences (add more rules as needed)
        if len(hash_str) >= 2:
            first_two_chars = hash_str[:2]
            two_char_data = historical_data[historical_data["Hash"].str.startswith(first_two_chars)]
            if not two_char_data.empty:
                predicted_multiplier = two_char_data["Multiplier"].mean() + 1.00

        if len(hash_str) >= 3:
            first_three_chars = hash_str[:3]
            three_char_data = historical_data[historical_data["Hash"].str.startswith(first_three_chars)]
            if not three_char_data.empty:
                predicted_multiplier = three_char_data["Multiplier"].mean() + 1.00
